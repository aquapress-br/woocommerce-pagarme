(function( $ ) {
	'use strict';
	$( function() { 		/* // Add pagarme data attributes to form and inputs		function InitPagarmeDataFields() {			$('.woocommerce-checkout, #add_payment_method').attr('data-pagarmecheckout-form', true);			$('#pagarme-card-number').attr('data-pagarmecheckout-element', 'number');			$('#pagarme-card-holder-name').attr('data-pagarmecheckout-element', 'holder_name');			$('#pagarme-card-expiry').attr('data-pagarmecheckout-element', 'exp_date');			$('#pagarme-card-cvc').attr('data-pagarmecheckout-element', 'cvv');		}				// Add pagarme data to myaccount method add		if( $(document.body).find('#add_payment_method').length ) {			if ( $( '#payment_method_wc_pagarme_creditcard' ).is( ':checked' ) || $( '.pagarme-card-option:checked' ).val() ) { 				InitPagarmeDataFields();			}		}		// Add pagarme data to checkout gateway		$(document.body).on('load updated_checkout', function(event, data) {			if ( $( '#payment_method_wc_pagarme_creditcard' ).is( ':checked' ) || $( '.pagarme-card-option:checked' ).val() ) { 				setTimeout(InitPagarmeDataFields(), 1000);			}		}); */   
		...
		$( document.body ).on( 'checkout_error', function() {
			$( 'body .pagarme-payment-form input' ).val( '' );
