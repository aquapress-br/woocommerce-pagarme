...
/**
 * \Aquapress\Pagarme\Migrations\Dokan_Deactivate_Seller class.
 *
 * @extends \Aquapress\Pagarme\Abstracts\Migration.
 */
class Dokan_Deactivate_Seller extends \Aquapress\Pagarme\Abstracts\Migration {

	/**
	 * @var string The target version for the migration.
	 */
	public $version = '1.0.0-dokan_deactivate_seller';
	...
	public function process( $current_version ): bool {
		...
		foreach ( $user_ids as $user_id ) {
			$recipient_id = get_user_meta( $user_id, 'pagarme_recipient_id', true );
			$sales_enabled = get_user_meta( $user_id, 'dokan_enable_selling', true );

			if ( empty( $recipient_id ) && 'yes' == $sales_enabled ) {
				update_user_meta( $user_id, 'dokan_enable_selling', 'no' );
				update_user_meta( $user_id, 'wc_pagarme_auto_deactivate_seller_processed', 'yes' );
			}
		}
